...
## 3- Convertir la variable Classe en character. Convertir la variable Prix en numerique.
$$\\$$
...
## 5- Afficher la ligne du passager ayant payé son billet le plus cher. Pour cela, trier la table par ordre de prix décroissant et afficher la 1ere observation.
...
## 6- Afficher la ligne du passager ayant payé son billet le moins cher. Pour cela, trier la table par ordre de prix décroissant et afficher la 1ere observation.
...
## 7- Faire un résumé statistique de la variable Prix. Faire également un boxplot de cette variable
$$\\$$
...
Le prix minimum est 0, la moyenne esr de 33.295 tandis que la médiane est de 14.454. Le prix maximum est de 512.329.
...
## 11-a Créer le projet R en faisant “File->New Project -> Existing Directory” et sélectionner votre répertoire de travail actuel (TP1). Et voilà!
...
## 12- Concaténer ces tables pour créer la table “titanic_client”. Au préalable, assurez-vous que les variables sont de même type pour pouvoir être concaténées.
...
```{r}
#- colnames(df.client.1)
#- colnames(df.client.2)
#- colnames(df.client.2)
...
## 13- A partir du fichier titanic_place.xlsx, créer une table “id14_15_16” contenant les individus associés aux identifiants “id14”, id15” et “id16”
...
## 14- Au sein de la table “id14_15_16”, convertir la variable Prix en numérique. Quel est maintenant le prix payé par nos individus ? Pourquoi ?
...
```{r,echo=FALSE, out.height="85%", out.width= "90%", fig.align='center', fig.cap=capFig("Dsitribution du prix du billet selon la classe")}
...
knitr::kable(table.nb.survivant.par.classe,caption = "Nombre de survivants selon la classe d'embarquement")
```
...
## 27- Calculer le taux de survie selon la variable Sexe_modif (taux de survie chez les hommes, chez les femmes et chez les enfants). 
...
```{r}
knitr::kable(stat.Prix.By.Classe.Sexe, caption = "Resultat de la question 9" )
```
...
|   On appelle VAI d’une série statistique, la plus petite valeur de la
série supérieure à la quantité Q1 − 1.5 (Q3 − Q1)


-  ***Définition de la VAS*** 

|   On appelle VAS d’une série statistique, la plus grande valeur de la
série inférieure à la quantité Q3 + 1.5 (Q3 − Q1)
...
#- anov <- summary(aov(Prix ~ Classe, data = df))
#- On enregistre les Sum square dataSq <- anov[[1]]
#- rap <- dataSq$SumSq[1]/sum(dataSq$SumSq)
```
